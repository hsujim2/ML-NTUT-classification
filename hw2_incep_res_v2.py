...
cnn_base = InceptionResNetV2(weights='imagenet',include_top=False,input_shape=(img_height,img_width,3))
#cnn_base = NASNetLarge(weights='imagenet',include_top=False,input_shape=(img_height,img_width,3))
cnn_base.trainable = True
classify = Sequential()
classify.add(Flatten())
classify.add(Dense(1024,activation='relu'))
...
#train module
checkpoint = ModelCheckpoint("/home/t107360144/inceptionresnetv2", monitor='val_categorical_accuracy', verbose=1, save_best_only=True,
mode='max')
...
model.save("/home/t107360144/ML_hw2/inceptionresnetv2")
...
select_df.to_csv('test_inception_resnet.csv',index= False)#存檔成csv
